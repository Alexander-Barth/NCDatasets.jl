...
function ncsave(filename,datadict)
    ...
    Dataset(filename,mode) do ds
        for (varname,value) in datadict
            _writevar(ds,varname,value)
        end
    end
end
...
Load all data from filenme in a dictionary.
"""
function ncload(filename)
    datadict = Dict{String,Any}()
    Dataset(filename,"r") do ds
        for (varname,value) in ds
...
function _writevar(ds,varname,value,dims)

    dimbylength = Dict((v,k) for (k,v) in ds.dim)
    ...
    v =         
        if varname in ds
            ...
            if eltype(value) == DateTime
                v = defVar(ds,varname,Float64,dims)
                v.attrib["units"] = default_timeunits
                v
            else
                v = defVar(ds,varname,eltype(value),dims)
            end
        end
...
function _writevar(ds,varname,value)
    dimbylength = Dict((v,k) for (k,v) in ds.dim)
    dims = fill("",(ndims(value),))

    for i = 1:ndims(value)
        if haskey(dimbylength,size(value,i))
            dims[i] = dimbylength[size(value,i)]
        else
            dims[i] = "dummy_dimension_$(size(value,i))"
        end
    end
    _writevar(ds,varname,value,dims)
end
